...
resource "google_compute_instance" "nifi-ca" {
    ...
    metadata_startup_script =   <<EOF
        ...
        sleep 10
        cd /root
        export PATH=$PATH:/usr/lib/jvm/jdk-11/bin && ${var.nifi-path}/nifi-toolkit-${var.nifi-version}/bin/tls-toolkit.sh client -D CN=${var.bh-hostname},OU=NIFI -c ${var.nifi-ca-hostname} -t ${var.ca-token}
        KEYSTORE_PASSWORD=`cat config.json | grep -o '"keyStorePassword" : "[^"]*' | grep -o '[^"]*$'`
        KEY_PASSWORD=`cat config.json | grep -o '"keyPassword" : "[^"]*' | grep -o '[^"]*$'`
        export PATH=$PATH:/usr/lib/jvm/jdk-11/bin && keytool -importkeystore -srckeystore keystore.jks -destkeystore keystore.p12 -srcstoretype jks -deststoretype pkcs12 -deststorepass $KEYSTORE_PASSWORD -srcstorepass $KEYSTORE_PASSWORD
